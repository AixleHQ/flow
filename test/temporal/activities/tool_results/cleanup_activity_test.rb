...
class Activities::ToolResults::CleanupActivityTest < ActiveSupport::TestCase
  setup do
  ...
  test "expires old completed results" do
    ...
    run_activity(Activities::ToolResults::CleanupActivity)
  ...
  test "a result that cannot be expired does not stop the others" do
    broken = create(:tool_result, :completed, tool: @tool)
    healthy = create(:tool_result, :completed, tool: @tool)
    ToolResult.where(id: [ broken.id, healthy.id ]).update_all(created_at: 40.days.ago)
    broken.update_column(:execution_id, "")

    result = run_activity(Activities::ToolResults::CleanupActivity)

    assert_equal 1, result[:expired]
    assert_equal "expired", healthy.reload.state
    assert_equal "completed", broken.reload.state
  end
end
