...
module Auth
  class PolicyUpdaterTest < ActiveSupport::TestCase
    ...
    test "an admin cannot disable the method their own live session was proved with" do
      ...
      auth_session = Auth::SessionService.start(user: @admin, provider: @password).auth_session
      updater = Auth::PolicyUpdater.new(company: @company, actor: @admin, auth_session: auth_session)
