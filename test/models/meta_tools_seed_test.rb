...
class MetaToolsSeedTest < ActiveSupport::TestCase
  ...
  test "all 28 aixle builder tool names exist as workflow tools" do
    found = Tool.where(kind: :workflow, name: AIXLE_BUILDER_TOOL_NAMES).pluck(:name).sort
  ...
  test "all seeded tools have a valid input_schema with type key" do
    tools = Tool.where(kind: :workflow, name: AIXLE_BUILDER_TOOL_NAMES)
    tools.each do |tool|
  ...
  test "every meta service file has a corresponding seeded tool" do
    ...
    seeded_names = Tool.where(kind: :workflow).where("name LIKE 'meta_%'").pluck(:name)
