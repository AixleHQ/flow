...
class Web::OidcSignInTest < ActionDispatch::IntegrationTest
  ...
  test "an OIDC sign-in appends its proof to a live session instead of replacing it" do
    ...
    sign_in_as(member)
    auth_session = AuthSession.live.find_by(user: member)

    state, nonce = start_and_capture_state
    build_token_stub(nonce)
    get oidc_callback_path(code: "the-code", state: state)

    assert_equal 1, AuthSession.live.where(user: member).count
    assert_includes auth_session.reload.proved_provider_ids, @provider.id
    assert_includes auth_session.proved_provider_ids, IdentityProvider.password.id
  end
end
