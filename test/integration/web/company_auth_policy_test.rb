...
class Web::CompanyAuthPolicyTest < ActionDispatch::IntegrationTest
  ...
  test "signing in with password leaves a live session carrying a password proof" do
    ...
    assert_equal [ @password.id ], user_session.proved_provider_ids
  end
  ...
  test "proving an accepted method at step-up restores entry" do
    ...
    user_session = UserSession.live.find_by(user: @user)
    user_session.proofs.destroy_all
    ...
    assert_includes user_session.reload.proved_provider_ids, @password.id
  end
  ...
  test "a wrong password at step-up is refused without ending the session" do
    ...
    assert UserSession.live.exists?(id: user_session.id)
  end
  ...
  test "proofs append across companies with disjoint policies, so step-up does not ping-pong" do
    ...
    assert_equal [ @password.id, @google.id ].sort, user_session.reload.proved_provider_ids.sort

    assert Auth::PolicyResolver.satisfied?(company: @company, user_session: user_session, user: @user)
    assert Auth::PolicyResolver.satisfied?(company: other, user_session: user_session, user: @user)
  end

  test "revoking a session ends access on the next request" do
    sign_in_as(@user)
    UserSession.live.find_by(user: @user).revoke!
