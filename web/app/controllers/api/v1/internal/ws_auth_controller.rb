    ...
    module Internal
      ...
      class WsAuthController < Api::V1::ApplicationController
        ...
        def show
          ...
          unless route_token
            Rails.logger.warn("[WsAuth] No route_token found in request")
            return head :bad_request
          end

          # Find session by route_token
          terminal_session = TerminalSession.find_by(route_token: route_token)
          unless terminal_session
            Rails.logger.warn("[WsAuth] Session not found for route_token: #{route_token}")
            return head :not_found
          end

          # Check user authentication
          unless current_user
            Rails.logger.warn("[WsAuth] No authenticated user for route_token #{route_token}")
          ...
          unless terminal_session.state.in?(%w[started running])
            Rails.logger.warn("[WsAuth] Session #{terminal_session.id} is not running (state: #{terminal_session.state})")
            return head :forbidden
          end

          # All checks passed - allow proxy
          Rails.logger.info("[WsAuth] Authorized user #{current_user.id} for session #{terminal_session.id}")
