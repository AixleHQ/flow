...
class TemporalServiceTest < ActiveSupport::TestCase
  ...
  test "create_schedule creates schedule when enabled" do
    schedule_def = OpenStruct.new(workflow: "unified_container_workflow", cron: "0 * * * *", enabled: true)
    workflow = OpenStruct.new(name: "UnifiedContainerWorkflow", owner: "container-queue")

    WorkflowService.stubs(:workflows).returns({ "unified_container_workflow" => workflow })

    mock_client = mock("client")
    mock_client.expects(:create_schedule).with("UnifiedContainerWorkflow", anything)
