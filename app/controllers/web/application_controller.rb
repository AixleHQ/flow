  ...
  inertia_share do
    ...
    if signed_in?
      shared.merge(
        ...
        projects: InertiaRails.always { Project.for_user(current_user).with_state(:active).with_computed_counts.includes(:project_collaborators).order(:name).map { |p| ProjectResource.new(p).to_h } }
      )
    else
      shared
    end
