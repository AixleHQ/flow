...
import { Box, Button, IconButton, Paper, Stack, TextField, Typography, type SxProps } from '@mui/material';
...
const getLanguageExtension = (ext: string) => {
  switch (ext) {
    ...
    case 'yaml':
      return yaml();
    case 'rb':
      // Ruby - use plain text for now
      return [];
    case 'sh':
    case 'bash':
      // Shell - use plain text for now
      return [];
    default:
      return [];
  }
  ...
  return (
    <Controller
      ...
      render={({ field, fieldState }) => (
        <Box>
          <Typography sx={styles.editorLabel}>Content {ext && `(${ext})`}</Typography>
