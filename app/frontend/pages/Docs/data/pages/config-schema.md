...
> **warning** **Rotate carefully.** Generate with `openssl rand -hex 32`. Rotate by re-encrypting stored values — there's no online rotation primitive yet.

All four are mandatory in production: `config/initializers/required_env.rb`
fails the boot if any is blank. Development and test fall back to fixed
non-secret values.
...
| `TRAEFIK_HTTP_BASE`     | no       | `http://localhost`                   | Public base URL agent session URLs are built on.           |
...
| `K8S_IMAGE_PULL_POLICY`           | no       | `IfNotPresent`                                         | Pod image pull policy.                                      |
...
| `K8S_TRAEFIK_VERIFY_TLS`          | no       | `false`                                                | `true` verifies Traefik's certificate when the app talks to it in-cluster. Off by default because that listener serves a self-signed certificate. |
...
| `CODER_REAP_ENABLED`               | no       | `true`   | `false` disables deletion of dead workspaces.                            |
...
| `AWS_S3_BUCKET`         | no       | `fake`  | Bucket used for uploads and asset storage.      |
...
| Variable        | Required | Default | Purpose                                                 |
| --------------- | -------- | ------- | ------------------------------------------------------- |
| `DOCS_LOGIN`    | no       | `user`  | Basic-auth username for `/docs`.                        |
| `DOCS_PASSWORD` | no       | unset   | Basic-auth password for `/docs`.                        |
