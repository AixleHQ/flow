...
const ProjectSessionArtifactsPage = () => {
  const { project, session, artifacts, alreadyReviewed } = usePage<{ props: Props }>().props as unknown as Props;

  const basePath = `/company/projects/${project.id}/sessions`;
  const sessionPath = `${basePath}/${session.id}`;
  ...
  const handleReview = useCallback(
    (action: 'save' | 'dismiss_all') => {
      ...
      router.post(
        `${sessionPath}/artifacts/review`,
        { decisions },
        {
          onSuccess: () => {
            notifications.show({
              message: action === 'dismiss_all' ? 'All outputs dismissed' : 'Outputs reviewed successfully',
              color: 'green',
              autoClose: 3000,
            });
          },
    ...
    [artifacts, selected, sessionPath],
  );

  return (
    <>
      <Head title={`Session Artifacts — ${project.name}`} />
      <Box maw={900} mx="auto">
        <Group mb="lg">
          <Button
            variant="subtle"
            ...
            onClick={() => router.visit(sessionPath)}
          >
            Back to Session
          </Button>
          <Text size="xl" fw={600}>
...
setPageLayout(ProjectSessionArtifactsPage, persistentProjectLayout);

export default ProjectSessionArtifactsPage;
