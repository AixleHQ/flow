...
describe('AssetsContent', () => {
  ...
  it('blocks the save and flags the field when the folder has invalid characters', async () => {
    renderPage(<AssetsContent {...baseProps} assets={[]} />);
    await userEvent.click(screen.getByRole('button', { name: /upload your first file/i }));
  ...
  // A folder is a nested path, and the folder is half of the asset's path — this exercises "/"
  // nesting together with trimming.
  it('posts a nested folder path, trimmed at the ends only', async () => {
    renderPage(<AssetsContent {...baseProps} assets={[]} />);
    await userEvent.click(screen.getByRole('button', { name: /upload your first file/i }));

    completeUpload([{ name: 'notes.md', key: 'cache/ddd444-notes.md' }]);
    await userEvent.type(await screen.findByPlaceholderText('Leave empty for root'), '  docs/sub  ');
    await userEvent.click(screen.getByRole('button', { name: /save 1 file/i }));

    expect(lastPostedAsset().folder).toBe('docs/sub');
  });
