              ...
              <Table.Tbody>
                ...
                {fileRows.map((asset) => (
                  <FileRow
                    ...
                    // No folder rows are ever rendered in "All files" view or while searching
                    // (folderRows is always []), so the drag handle would have nowhere to drop —
                    // show it only when a folder is actually on screen to drop onto.
                    draggable={canExecute && !selection.bulkMode && folderRows.length > 0}
                  />
                ))}
              </Table.Tbody>
            </Table>
