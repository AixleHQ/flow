...
| `cursor_cli` | **answered 2026-09-18 by a live login inside the built image.** Our call was wrong twice — `authenticator.cursor.sh` is a real OAuth server, but its token endpoint is `/oauth2/token`, not the `/oauth/token` we posted to (hence the HTML 404); corrected, it answers `invalid_client` for our client id and for cursor.com's own WorkOS client. The stored token is a Cursor session JWT (`aud=cursor.com`, `type=session`), which WorkOS rejects as `invalid_grant`, and the CLI never exchanges it — both the July and September bundles carry only `/auth/poll`, `/auth/exchange_user_api_key`, `/auth/cursor_dev_session_token`, and their refresh policy consumes an API key. Eight further route guesses on api2: all 404. Measured lifetime 60 days (issued 2026-09-18, exp 2026-11-16). Declared `reauth_only`; the dead refresh code is gone, and with it ~10 404s an evening from the web process | done |
