...
2. They supply a personal access token carrying **Member Entitlement Management (read & write)**, which is used to call an endpoint only an organization administrator can call. That is the same authority that would otherwise add the application to the organization by hand, so holding it is the claim being made. It also needs **Security (manage)**, spent on one access control entry rather than on any resource: *View subscriptions* + *Edit subscriptions* on the ServiceHooks namespace (`cb594ebe-87dd-4fc9-ac2c-6a10a4c92046`), token `PublisherSecurity/{projectId}`, so the application can create its own Service Hooks from then on — at connect time and on every later re-test.

Creating the subscriptions with the administrator's token instead was the obvious alternative and is worse: Azure does not trigger a subscription whose creator has lost access to the resource, so the events would stop on the day that person left and nothing would say so. A webhook that quietly stops is worse than one that was never created, and the recovery sweep would hide it. The grant is best effort — an organization that refuses it gets a connection whose CI gates resolve on the sweep instead of on events.
...
**Send no filter but the project.** Everything else in `publisherInputs` is a filter, and Azure does not reject an unrecognized filter VALUE: it accepts the subscription, reports it `enabled`, records no error — and matches nothing, forever. This shipped as `buildStatus: "Completed"`, which reads perfectly and is not one of the values Azure accepts (`""`, `Succeeded`, `PartiallySucceeded`, `Failed`, `Stopped`; ask `_apis/hooks/publishers/tfs/eventtypes`). The only symptom was silence: three healthy subscriptions, zero deliveries, gates closing ten minutes late on the recovery sweep. Filtering by result would be wrong even spelled correctly — a gate has to hear about a failed build too, or it waits for a verdict that already happened.

Handle duplicate/out-of-order delivery and inspect disabled/failing subscriptions; add bounded polling reconciliation for pending gates. Delivery failures can place a subscription on probation, during which new events can be lost, or disable it. Do not promise lossless delivery. [Service Hook troubleshooting](https://learn.microsoft.com/en-us/azure/devops/service-hooks/troubleshoot?view=azure-devops). Automated setup needs *View subscriptions* + *Edit subscriptions*, which Azure gives project administrators only — §5.2 grants them to the application once, with the onboarding token, so the subscriptions belong to the application rather than to a person who may leave.
