...
# =============================================================================
# Open Code Container Entrypoint
#
# Starts:
...
#   - Or local models (no key required)
# =============================================================================
...
# Set HOME for Open Code
export HOME="/home/opencode"
export PATH="$HOME/.opencode/bin:$PATH"

echo ""
echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Open Code Interactive Session${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
echo ""
...
if [ -n "$OPENAI_API_KEY" ]; then
    echo -e "${GREEN}✅ OPENAI_API_KEY configured (OpenAI provider available)${NC}"
fi

if [ -n "$ANTHROPIC_API_KEY" ]; then
    echo -e "${GREEN}✅ ANTHROPIC_API_KEY configured (Anthropic provider available)${NC}"
fi

if [ -z "$OPENAI_API_KEY" ] && [ -z "$ANTHROPIC_API_KEY" ]; then
    echo -e "${YELLOW}⚠️  No API keys set - Open Code will use local models or require login${NC}"
fi
...
# Default command is opencode, can be overridden with TTYD_CMD env var
TTYD_CMD="${TTYD_CMD:-opencode}"
...
echo -e "    opencode - Start Open Code assistant"
