...
#
# Starts:
#   - ttyd (web terminal) on port 7681 running Cursor Agent
#   - File watcher service on port 4040
#
# Cursor Auth:
#   Will prompt for login on first use or use existing session
# =============================================================================
...
# -----------------------------------------------------------------------------
# Start ttyd Web Terminal with Cursor Agent
# -----------------------------------------------------------------------------
echo -e "${CYAN}🖥️  Starting web terminal on port ${TTYD_PORT}...${NC}"

# Default command is 'agent' (Cursor Agent CLI)
# Can be overridden with TTYD_CMD env var
TTYD_CMD="${TTYD_CMD:-agent}"

# Start ttyd with Cursor Agent
...
echo -e "    agent    - Start Cursor Agent (default)"
echo -e "    /model   - Switch AI model"
