...
**User Outcome:** Reusable agent configurations with personas for workflows and sessions.

### Story 5.1: Create Agent Configuration

As a company admin,
I want to create agent configurations with persona details,
So that agents can be reused across workflows and sessions.

**Acceptance Criteria:**
- Can create agent with: name, title, icon (emoji), persona, communication_style, principles
- Agent scoped to company or project level
...
- Creates Agent record with imported data
- Shows preview before import

### Story 5.3: Select Agent for Session

As a user,
I want to select an agent when starting a session,
So that the agent's persona is applied to my interaction.

**Acceptance Criteria:**
- Session start shows available agents
- Can select from company + project agents
- Selected agent's persona injected as system prompt
- Agent selection saved with session record

### Story 5.4: Agent Scoping (Company/Project)

As a system,
I want agents scoped to company or project level,
So that project-specific agents override company defaults.

**Acceptance Criteria:**
- Agents have optional project_id (nil = company-wide)
- Session shows merged list (project + company)
...
## Epic 7: MCP Servers (Phase 3)

Admins can configure MCP servers to expose tools to agents.

**FRs covered:** FR49, FR50, FR51, FR52

**Phase:** 3 (Depends on: Epic 6 Tools)

**User Outcome:** Tools exposed to CLI agents via MCP protocol.

### Story 7.1: Configure MCP Server

As a company admin,
I want to configure MCP servers at company/project level,
So that tools can be exposed via MCP protocol.

**Acceptance Criteria:**
- Can create MCP server with: name, description, transport (stdio/http/websocket), config
- MCP server scoped to company or project
- Can enable/disable MCP server

### Story 7.2: Select Tools for MCP Server

As a company admin,
I want to select which tools are exposed via MCP server,
So that agents only see relevant tools.

**Acceptance Criteria:**
- Can select tools to expose from available tools
- Can optionally rename tool for MCP exposure
- MCP server exposes selected tools via MCP protocol

### Story 7.3: MCP Server Lifecycle (Per-Session)

As a system,
I want MCP server to start alongside agent session,
So that tools are available during the session.

**Acceptance Criteria:**
- MCP server starts when session starts
- Runs alongside agent container
- Stops when session ends
- CLI agents connect to MCP server automatically
