...
class CreateSessionAdmissions < ActiveRecord::Migration[8.1]
  def change
    create_table :session_admission_policies do |t|
      ...
      t.integer :project_default, null: false, default: 2
      t.integer :user_default, null: false, default: 2
      t.integer :revision, null: false, default: 1
      t.timestamps
    end
    add_check_constraint :session_admission_policies, "id = 1 AND project_default > 0 AND user_default > 0 AND (installation_limit IS NULL OR installation_limit > 0)", name: "valid_session_policy"
    reversible { |dir| dir.up { execute "INSERT INTO session_admission_policies (id, created_at, updated_at) VALUES (1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)" } }
